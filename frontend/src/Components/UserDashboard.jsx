...
const UserDashboard = ({ userEmail }) => {
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            try {
                const userRes = await axios.get(`${import.meta.env.VITE_APP_REQUEST_API}/user/${userEmail}`);
                console.log("userRes", userRes.data);
                setUser(userRes.data.users);

                const blogsRes = await axios.get(`${import.meta.env.VITE_APP_REQUEST_API}/MyBlogs/${userEmail}`);
                console.log("blogsRes", blogsRes.data);
                setBlogs(blogsRes.data);

                const savedRes = await axios.get(`${import.meta.env.VITE_APP_REQUEST_API}/savedBlogs/${userEmail}`);
                    ...
                    <div className="bg-white shadow-md p-6 rounded">
                        ...
                        <div className="space-y-3 max-h-[300px] overflow-y-auto">
                            {savedBlogDetails.map(blog => (
                                <Link to={`/show/${blog._id}`} key={blog._id} className="flex items-center gap-4 p-2 border-b hover:bg-gray-50 rounded">
                                    <img src={`${import.meta.env.VITE_APP_REQUEST_API}/${blog.image}`} alt="saved" className="w-16 h-16 object-cover rounded" />
                                    <div>
